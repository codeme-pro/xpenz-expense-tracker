...
function ReportDetail() {
  ...
  const imageResults = useQueries({
    queries: expensesWithImages.map((e) => ({
      queryKey: [...queryKeys.scanUrl(e.scanFilePath!), 'print'],
      queryFn: () => fetchScanSignedUrlForPrint(e.scanFilePath!),
    })),
  })
  ...
  const addExpenses = useMutation({
    mutationFn: (ids: string[]) => addExpensesToReport(reportId, ids),
  ...
  const addMileageEntries = useMutation({
    mutationFn: (ids: string[]) => addMileageEntriesToReport(reportId, ids),
